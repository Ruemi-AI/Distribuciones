...
document.addEventListener('DOMContentLoaded', () => {
    // Mostrar modal de Pronóstico
    document.getElementById('pronosticoBtn').addEventListener('click', () => {
        console.log('Botón Pronóstico clickeado');
    ...
    // Obtener demanda actual (web scraping)
    document.getElementById('obtenerDemanda').addEventListener('click', async () => {
        const ubicacion = document.getElementById('ubicacion').value;
        const resultadoDiv = document.getElementById('resultadoDemanda');
        
        resultadoDiv.innerHTML = "Obteniendo datos...";
        
        try {
            const demandaActual = await obtenerDemandaDesdeWeb(ubicacion);
            resultadoDiv.innerHTML = `<strong>Demanda actual:</strong> ${demandaActual} MW`;
        } catch (error) {
            resultadoDiv.innerHTML = "Error al obtener la demanda.";
        }
    });

    // Predecir demanda usando TensorFlow.js
    document.getElementById('predecirDemanda').addEventListener('click', async () => {
        const ubicacion = document.getElementById('ubicacion').value;
        const hora = document.getElementById('hora').value;
        const resultadoDiv = document.getElementById('resultadoDemanda');

        resultadoDiv.innerHTML = "Generando pronóstico...";

        try {
            const pronostico = await predecirDemanda(ubicacion, hora);
            resultadoDiv.innerHTML = `<strong>Pronóstico para ${hora}:</strong> ${pronostico.toFixed(2)} MW`;
        } catch (error) {
            resultadoDiv.innerHTML = "Error en la predicción.";
        }
    });

    // Función para hacer web scraping (Ejemplo simple)
    async function obtenerDemandaDesdeWeb(ubicacion) {
        const urls = {
            mexico: "https://www.cenace.gob.mx",
            texas: "https://www.ercot.com",
            california: "https://www.caiso.com"
        };

        const response = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(urls[ubicacion])}`);
        const data = await response.json();
        const parser = new DOMParser();
        const doc = parser.parseFromString(data.contents, "text/html");

        // Extraer la demanda (esto varía según el sitio)
        const demanda = doc.querySelector(".demanda-actual"); // Cambiar según la estructura real
        return demanda ? demanda.textContent.trim() : "No disponible";
    }

    // Función para predecir la demanda con TensorFlow.js
    async function predecirDemanda(ubicacion, hora) {
        // Cargar el modelo
        const modelo = await tf.loadLayersModel('modelo_demanda.json'); // Reemplazar con el path correcto

        // Convertir la hora en un número (ejemplo: 14:30 → 14.5)
        const [horas, minutos] = hora.split(":").map(Number);
        const horaNumerica = horas + minutos / 60;

        // Codificar la ubicación (Ejemplo: México = 0, Texas = 1, California = 2)
        const ubicaciones = { mexico: 0, texas: 1, california: 2 };
        const entrada = tf.tensor2d([[ubicaciones[ubicacion], horaNumerica]]);

        // Hacer la predicción
        const resultado = modelo.predict(entrada);
        const valorPredicho = await resultado.data();

        return valorPredicho[0]; // Retorna el valor predicho en MW
    }
});
